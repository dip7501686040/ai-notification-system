...
Learn from historical behavior over time 6. Product Objectives
...
Offer AI-assisted decision making. 7. Success Metrics (KPIs)
Metric Target
...
Notification retry success >99% 8. Stakeholders
Internal
Platform Engineers
SRE Team
AI Engineering Team
...
Operations Teams
Business Managers 9. Target Customers
Small Businesses
...
- JWT / OAuth2 authentication, API keys for external ingest
...
(Full detail: §2.13 below is now just a cross-reference to this list — see also §2.14 Compliance.)
...
See Security under §2.3 Non-Functional Requirements above — full list, not repeated here.
...
# Chapter 3 — Architecture & Communication Design

---

## 3.1 Guiding principle

A microservice architecture, chosen deliberately to demonstrate distributed-systems and production-architecture practice, not just to ship a notification tool. Each interaction uses whichever communication style actually fits it, rather than one protocol everywhere:

| Communication     | Used for                             | Why                                     |
| ----------------- | ------------------------------------ | --------------------------------------- |
| REST              | External API (browser, integrations) | Public, simple, SDK-friendly            |
| gRPC              | Internal service-to-service          | Fast, strongly typed, low latency       |
| Events (RabbitMQ) | Async business workflows             | Decoupling, independent retries/scaling |

GraphQL was considered for dashboard data aggregation but not adopted: the frontend's actual needs (list/detail views, not deep nested graphs) are served well by REST + TanStack Query, and a second query protocol wasn't justified once built.
...
Client / API key
 ...
 api-gateway ──gRPC──► identity-service   (JWT / API-key auth)
      │       ──gRPC──► tenant-service     (multi-tenancy, membership, API keys, billing)
      │  gRPC: CreateEvent
      ▼
 event-service ──publish──► event.created
                        ...
                        rule-engine-service (evaluate rules)
                         ...
                         event.rule.matched
                             ...
                             ai-service (LLM: severity / impact / duplicate)
                                  │ publish                    ▲
                                  ▼                            │ gRPC (predictions)
                         event.ai.completed         prediction-service (Python/ML)
                                  │
                                  ▼
                     ...
                     ┌────────────┼───────────────────────┐
                     ▼            ▼                        ▼
             notification.sent  notification.retry   notification.dashboard.push
                     │           notification.dead          │
                     └──────┬───────────┬───────────────────┘
                            ▼           ▼
                    analytics-service   api-gateway (WebSocket relay to browser)

 Parallel async taps (data collection, never in the critical path):
   audit-service     ← notification.sent, event.ai.completed, audit.created (rule/API-key CRUD, login)
   analytics-service ← event.created, notification.sent, notification.dead, notification.dashboard.push

 Every service ──► OpenTelemetry ──► Prometheus / Loki / Jaeger ──► Grafana
```

Only auth (api-gateway → identity-service), tenant/membership lookups (→ tenant-service), event creation (api-gateway → event-service), and template rendering (notification-service → template-service) are synchronous gRPC — each needs an answer before the caller can proceed. Everything from `event.created` onward is async: no service calls the next stage directly, each publishes what it produced and moves on.

## 3.3 Services

| Service              | Owns                                                                                                                                                                 | Protocol surface                     |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| api-gateway          | Auth/routing, no business logic                                                                                                                                      | REST in, gRPC out                    |
| identity-service     | Users, JWT, OAuth                                                                                                                                                    | gRPC                                 |
| tenant-service       | Orgs, membership, RBAC, API keys, billing (Stripe)                                                                                                                   | gRPC                                 |
| event-service        | Event ingestion, `event.created`                                                                                                                                     | gRPC + publish                       |
| rule-engine-service  | Rule evaluation, `event.rule.matched`                                                                                                                                | consume + publish                    |
| ai-service           | LLM analysis (summary/severity/impact/duplicate via RAG), `event.ai.completed`                                                                                       | consume + publish                    |
| prediction-service   | In-house ML models (Python: XGBoost/PyTorch/TensorFlow) — distinct from ai-service, which calls _external_ LLMs; separate because the two scale and fail differently | gRPC, called by ai-service           |
| notification-service | Notification lifecycle, retry/dead-letter, `notification.created`                                                                                                    | consume + publish                    |
| template-service     | `{{variable}}` template rendering                                                                                                                                    | gRPC, called by notification-service |
| channel-service      | Real delivery: email (SMTP), webhook, dashboard (WebSocket push via api-gateway)                                                                                     | consume + publish                    |
| analytics-service    | Daily aggregate stats                                                                                                                                                | consume-only                         |
| audit-service        | Audit trail across 5 action types                                                                                                                                    | consume-only                         |
| web                  | Next.js SaaS dashboard                                                                                                                                               | REST + WebSocket client              |

## 3.4 RabbitMQ (single durable topic exchange, `platform`)

`event.created → event.rule.matched → event.ai.completed → notification.created → {notification.sent | notification.retry | notification.dead | notification.dashboard.push}`, plus `audit.created` (published independently by rule-engine-service, identity-service, and tenant-service on auditable actions). Every service subscribes only to the routing keys it cares about — no service is aware of anything downstream of what it publishes.

## 3.5 Database strategy

Database per service, no cross-service SQL joins — identity, tenant, event, rule-engine, notification, ai, analytics, and audit each own their own Postgres schema. Cross-service reads go through gRPC or the event bus, never a shared table.

## 3.6 Why RabbitMQ, not Kafka

RabbitMQ was chosen for reliable work queues, flexible routing, dead-letter handling, and lower operational complexity than Kafka needs for this workload. Kafka would be revisited only if throughput moved into the hundreds-of-thousands-of-events/sec range or long-term event replay became a requirement — neither applies at this scale.

---

# Chapter 4 — Development Process Log
...
Features 7. ✅ API Gateway + Auth Service end-to-end (register/login/JWT/OAuth).
...
8. ✅ Tenant Service (organizations, membership, RBAC).
   ...
   tries to update tenant settings, 401 with no bearer token, 404 (not 403) for a non-member requesting a tenant by id so existence isn't
...
9. ✅ Event Service (ingestion, first RabbitMQ producer).
   ...
   _and_ tenant membership -- the latter checked over real gRPC against
...
10. ✅ Route every public API through api-gateway only.
    ...
    exposed REST directly (ports 8001/8002/8003) _in addition to_
...
11. ✅ Rule Engine Service (first RabbitMQ consumer, event.created ->
    ...
    This is the first RabbitMQ _consumer_ in the codebase -- everything
    ...
    forgotten): no dead-letter queue, no action _execution_ (that's
...
/ai-analyses/by-event/:eventId`) via api-gateway, same shape as every
    ...
    duplicate reported under a _different_ event type/wording, which an
...
15. ✅ Template Service ({{variable}} rendering for FR-6, wired into
    ...
    so there was nothing to substitute variables _from_. Added those
...
requester` alone is sufficient authorization, and naturally covers
    both a user's own logins _and_ their own rule changes across every
...
--build`. Every container in this pass was rebuilt and health-checked
    ...
    alone produced _no_ row, matching the login-only scope). Created,
...
tenant-service` fully implements `owner`/`admin`/`member`
    ...
    `allowedRoles?: string[]` parameter -- this is the _authoritative_
    ...
    `TenantRolesGuard` -- as a fast-fail wherever `tenantId` _is_
...
19. ✅ FR-10 API Management: API Keys (create/rotate/revoke) + rate
    ...
    `EventsController`'s `create()` now accepts _either_ auth mode: an
...
21. ✅ Event ingestion guard: reject `POST /events` synchronously when no
    ...
    analysis respectively), so rejecting ingestion whenever a _specific_
